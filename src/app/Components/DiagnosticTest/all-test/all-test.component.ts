...
export class AllTestComponent implements OnInit {
  ...
  constructor(private testService: DiagnosticTestServiceService, private router: Router) { }
  ngOnInit(): void {
    this.getAll();
  }
  removeDiagnosticTest(diagnosticTestid:number){
    console.log(diagnosticTestid)
    this.testService.deleteTest(diagnosticTestid).subscribe((data:DiagnosticTest) => {
